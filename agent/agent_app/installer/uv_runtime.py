...
def discover_uv(*, operator: OperatorIdentity, override: Path | None) -> UvRuntime:
    ...
    if override is not None:
        ...
        if _is_executable(override):
            return UvRuntime(bin_path=override, source="explicit", searched=searched)
    ...
    if which:
        ...
        return UvRuntime(bin_path=Path(which), source="path", searched=searched)

    return UvRuntime(bin_path=None, source="missing", searched=searched)
...
def build_upgrade_command(
    ...
    *,
    ...
    if os_name == "Linux":
        ...
        return ["sudo", "-u", operator.login, "-H", "env", home_arg, bin_path, "tool", "upgrade", package_spec]
    if os_name == "Darwin":
        return ["sudo", "-u", operator.login, "-H", "env", home_arg, bin_path, "tool", "upgrade", package_spec]
