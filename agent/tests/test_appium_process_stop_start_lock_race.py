...
async def test_stop_holds_start_lock_during_process_teardown() -> None:
    ...
    the Appium process teardown (``proc.wait()``) that runs *before* the
    appium-proc/launch-spec dicts are popped. Without the fix, a concurrent
    ``start()`` for the same port can recreate ``_launch_specs[port]`` and
    discard the ``_intentional_stop_ports`` flag mid-stop. With the fix, any
    other coroutine attempting ``_start_lock.acquire()`` blocks until
    ``stop()`` completes.
    """
    ...
    async def fake_wait() -> None:
        # Signal that stop() is past _intentional_stop_ports.add and is
        # inside the Appium process teardown — this is the moment a concurrent
    ...
    fake_appium_proc.wait = fake_wait
