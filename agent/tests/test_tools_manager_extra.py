...
def test_node_provider_command_and_fnm_discovery_helpers() -> None:
    ...
    with (
        ...
        patch("agent_app.tools.manager._is_executable", side_effect=lambda path: path == "/usr/local/bin/fnm"),
    ):
        assert _find_fnm_binary() == "/usr/local/bin/fnm"

    with (
        patch("agent_app.tools.manager.shutil.which", return_value=None),
        patch("agent_app.tools.manager._is_executable", side_effect=lambda path: path == "/opt/homebrew/bin/fnm"),
    ):
    ...
    with patch("agent_app.tools.manager.os.path.isdir", side_effect=lambda path: path.endswith("/aliases/default/bin")):
