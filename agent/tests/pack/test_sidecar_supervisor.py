...
def _handles(sup: SidecarSupervisor) -> dict[tuple[str, str, str], SidecarHandle]:
    return sup._handles
...
async def test_start_when_adapter_returns_not_ok_does_not_schedule_poll() -> None:
    ...
    try:
        ...
        handle = _handles(sup)[("vendor-fake", "1.0.0", "tunnel")]
...
async def test_stop_cancels_poll_task_and_calls_adapter_stop() -> None:
    ...
    try:
        ...
        handle_before = _handles(sup)[("vendor-fake", "1.0.0", "tunnel")]
        ...
        # handle removed
        assert ("vendor-fake", "1.0.0", "tunnel") not in _handles(sup)
    finally:
        await sup.shutdown()
...
async def test_shutdown_cancels_all_poll_tasks() -> None:
    ...
    for handle in _handles(sup).values():
