...
async def test_detect_capabilities_uses_contract_version_without_pull_marker() -> None:
    ...
    assert capabilities["orchestration_contract_version"] == 4
...
async def test_capabilities_snapshot_refreshes_only_when_missing_or_forced() -> None:
    ...
    with patch.object(
        ...
        assert await cache.get_or_refresh() == {**first_snapshot, "orchestration_contract_version": 4}
        assert await cache.get_or_refresh() == {**first_snapshot, "orchestration_contract_version": 4}
        assert await cache.get_or_refresh(force=True) == {**second_snapshot, "orchestration_contract_version": 4}
