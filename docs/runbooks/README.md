...
- `appium-node-desync.md`
  - Recovery when a node's derived `effective_state` (e.g. `stopped`) diverges from the host agent's actual running Appium process — orphaned/stuck process or an unsatisfied restart watermark, converged by the scheduler's `host_sweep` loop.
