...
| `GRIDFLEET_UVICORN_WORKERS` | `1` | backend container entrypoint | Number of Uvicorn worker processes. Leader-owned background loops run only on the elected leader (Postgres advisory lock), so additional workers add API-serving capacity without duplicating loops. Each worker opens its own DB pool — keep `WORKERS x (DB_POOL_SIZE + DB_MAX_OVERFLOW)` under Postgres `max_connections` (default 100). Values `<= 1` or non-numeric run a single worker (current behavior). |
