...
| `GET` | `/api/devices/{device_id}/config` | Read device config, optionally filtered and unmasked | `keys`, `reveal` | config object |
...
| `GET` | `/api/devices/{device_id}/config/history` | Read masked config audit history | `limit` | config-audit entries |
