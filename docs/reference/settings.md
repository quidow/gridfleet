...
| `general.session_viability_failure_threshold` | `general` | `int` | `3` | `1..20` | Up to this many probe attempts per scheduled viability pass; the pass retries ~10 s apart and escalates the device to lifecycle recovery when the consecutive-failure count reaches this value (residual failures from earlier passes count toward it, so a pass may run fewer attempts) |
