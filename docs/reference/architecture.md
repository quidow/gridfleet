...
Session-viability and device-verification probes create a short-lived session directly against the device's Appium server, with no Grid involved. Every probe has a `Session` row **from birth** (`app.sessions.service_probes`): committed `pending` before the `POST /session` (the orphan sweep's pending-device sparing covers the create window, exactly as for backend-owned client creates), promoted to `running` with the real Appium session id when create returns, and stamped terminal on completion. The row is the probe's claim on its device — the allocator's live-session gates and the orphan sweep's known-ids match read it; there is no separate probe lock or in-process registry. A probe orphaned by a scheduler crash is an ordinary crash-orphaned row: the allocation reaper fails a stale `pending` claim, and the liveness sweep closes a `running` one and terminates its Appium session. Probe rows carry `test_name == "__gridfleet_probe__"` and a `requested_capabilities["gridfleet:probeCheckedBy"]` source attribution (`scheduled`, `manual`, `recovery`, or `verification`).

Probe rows **claim but do not mask** (D14): they gate allocation-class actions like any live session, but are excluded from the `busy` projection input (`masking_live_session_predicate` in `app.sessions.live_session_predicate`), so probe cadence never flips a device's operational-state ledger or emits edge events.
...
Probe rows are excluded from success-rate, throughput, utilization, error breakdown, and heatmap analytics via the existing `exclude_non_test_sessions` / `exclude_non_success_metric_sessions` filters keyed on `test_name`. Probe persistence does **not** emit `session.started` or `session.ended` events — the event stream sees no probe traffic. Operators surface probes — including currently-running ones — on the Sessions page via the `include_probes` query parameter (off by default). Probes have their own retention window via `retention.probe_sessions_days` (default 7 days).
...
| Testkit per-attempt preparation-failure timeout (35 s, `PREPARATION_FAILURE_TIMEOUT_SEC` in `testkit/gridfleet_testkit/client.py`) | Backend ASGI request watchdog (`request_timeout_sec` = 30 s, `backend/app/core/config.py`) | A > B, or the client abandons the request before the watchdog can return its classified response and the retry becomes a blind re-send against an in-flight transaction | backend | parity test `backend/tests/contracts/test_timeout_lattice_parity.py` (loads the testkit constant by file path) |

(Two rows are backend↔agent and backend↔testkit rather than backend↔router; they are the same class of rule and this table is the designated single home, so they live here too.)
