...
Every other `insecure_features` entry widens the trust boundary around *session clients* rather than pack authors: the feature applies to whatever the allocated Appium server accepts from whoever holds the session. The curated `appium-uiautomator2` pack requests `uiautomator2:adb_shell`, which enables the driver's `mobile: shell` extension command — any client that can allocate an Android device can then run arbitrary commands on it with the privileges of `adb shell`. There is no operator-side override; `insecure_features` is read from the selected release's stored manifest, so a lab whose session clients are not trusted must fork the manifest, drop the entry, and upload the result as its own pack.
