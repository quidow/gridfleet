...
| --- | --- | --- | --- | --- |
| POST | `/api/hosts/register` | bootstrap, then periodic enrollment refresh (`RegistrationService.run`, `AGENT_REGISTRATION_REFRESH_INTERVAL_SEC`, default 300 s) | enrollment only: identity/ip/os_type/agent_port/host_info, plus capabilities carried solely as the 426 contract-gate input (not persisted). It does not write `agent_version`, `capabilities`, `Host.status`, or `last_heartbeat` — those are push-owned runtime facts | 2xx, returns `Host` row id |
| GET | `/agent/driver-packs/desired` | `PackStateLoop` (~10 s) | desired pack list for this host | 200 → `{packs: [...]}` |
| POST | `/agent/hosts/status` | consolidated status-push loop (`AGENT_STATUS_PUSH_INTERVAL_SEC`, default 10 s) | the one status-bearing channel: Appium nodes/processes, restart events, start failures, pack status, host telemetry, agent version/capabilities in a single envelope (`HostStatusPush`); stamps `Host.last_heartbeat` and never writes `Host.status` (the sweep's edge detector owns the online/offline ledger). Rejected with 426 if the capabilities block reports a pre-v6 orchestration contract | 204 |
...
`NodeStateLoop` starts whenever the agent has a backend URL. It starts, stops, reconfigures, and reaps local orphan processes from the desired projection on every poll (5 s default, or immediately on a wake poke). A node's `restart_requested_at` watermark (`docs/design/02-node-lifecycle.md`) forces one respawn per agent process; the backend confirms the watermark is satisfied by comparing it against the `started_at` the agent reports on each running-node entry — both `/agent/health` and the consolidated status push carry that entry via `process_snapshot()`. The agent advertises `orchestration_contract_version: 6` unconditionally; the backend validates that contract at registration **and** on every status push (426 on a pre-v6 block), then always writes desired state and pokes (see `docs/reference/architecture.md` for the contract-version floor).
