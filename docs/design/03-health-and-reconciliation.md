...
| `host_sweep` node-health fold | Watermark-gated fold (`host_sweep.observation_fold`), one fold per changed stamp; runs only for hosts the same pass proved alive | Push `node_health` section (agent produces it locally on a 30 s constant) | `AppiumNode.consecutive_health_failures`, `AppiumNode.health_running`, `AppiumNode.health_state`, `AppiumNode.last_health_checked_at`, lifecycle JSON; on max failures registers auto-recovery intents via `IntentService.register_intents_and_reconcile` (does not write `operational_state` directly) | node-health counter, auto-restart trigger |
| `host_sweep` connectivity fold | Watermark-gated fold (`host_sweep.observation_fold`), one fold per changed stamp; per host after convergence | Push `device_health` section (agent produces it locally on a 60 s constant) | `Device.device_checks_*`, `Device.emulator_state`, lifecycle JSON; calls `IntentService.reconcile_now` / `register_intents_and_reconcile` (does not write `operational_state` directly) | `Device.device_checks_*` |
...
| `host_sweep` host-resource-telemetry fold | Watermark-gated fold (`host_sweep.observation_fold`), one fold per changed stamp; per host after convergence | Push `host_telemetry` section (folded no more often than every 60 s) | `host_resource_samples` table (model `HostResourceSample`) | host telemetry rows |
| `host_sweep` hardware-telemetry fold | Watermark-gated fold (`host_sweep.observation_fold`), one fold per changed stamp; per host after convergence | Push `device_telemetry` section (agent produces it locally on a 300 s constant) | `Device.battery_*`, `hardware_health_status` | hardware fields |
| `host_sweep` property-refresh fold | Watermark-gated fold (`host_sweep.observation_fold`), one fold per changed stamp; per host after convergence | Push `device_properties` section (agent produces it locally on a 600 s constant) | `Device.os_version`, `software_versions`, etc. | device property fields |
...
Device connectivity folded into `host_sweep` too (it is no longer a standalone loop). It is a watermark-gated fold of the push's `device_health` section (`host_sweep.observation_fold`), run per host after convergence; its apply metrics still report under the `device_connectivity` label. Because the observation arrives in the push rather than a dial, the fold does no probing — a changed stamp folds exactly once, and a host with no fresh section is skipped. Soak criterion: `background_loop_overrun_total{loop="host_sweep"}` stays near zero.

Host-resource telemetry, hardware telemetry, and property refresh folded into `host_sweep` too (they are no longer standalone loops), each now a watermark-gated fold of a pushed section (`host_telemetry`, `device_telemetry`, `device_properties`) rather than a `SweepStage` global stage. The fold re-feeds the same fact writers (`apply_host_resource_sample`, `apply_telemetry_sample`, `apply_pack_device_properties`) from the push instead of a dial; each folds exactly once per changed per-host stamp.
...
- `AppiumNode.consecutive_health_failures`: owned by the `host_sweep` node-health fold
...
| `host_sweep.observation_fold` | `host_sweep` observation folds | per-host stamp watermark per pushed section, so each observation folds exactly once |
