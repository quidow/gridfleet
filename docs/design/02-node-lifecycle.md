...
| Component | Role |
| --- | --- |
| `RemoteNodeManager` (`backend/app/services/node_manager.py:101-327`) | Operator-triggered transitions — start, stop, restart |
| `node_manager_state` (`backend/app/services/node_manager_state.py`) | Sole writer of `AppiumNode.state`, syncs the health snapshot |
| `node_manager_remote` (`backend/app/services/node_manager_remote.py`) | HTTP dispatch to the agent + `restart_node_via_agent` (loop-driven path) |
...
## Flow A — Operator start (`RemoteNodeManager.start_node`)

```mermaid
sequenceDiagram
    autonumber
    participant API as API router
    participant NM as RemoteNodeManager.start_node
    participant State as node_manager_state
...
- **Readiness gate** (`node_manager.py:174`) refuses if `is_ready_for_use_async` says no.
- **Owner allocation first, port second** — the allocator owns ports because they are part of the host's parallel-resource pool (`appium_resource_allocator.get_or_create_owner_capabilities`). On failure during start, the allocation is released by the same try/except in `_start_with_owner` (`node_manager.py:162-166`).
- **Port conflict retry** — if the agent rejects with "already in use", the manager continues to the next candidate port (`node_manager.py:140-161`). Conflicts on the managed range come from external listeners or stale agent state; trying the next port is correct.
- **Readiness wait** — `_wait_for_remote_appium_ready` (`node_manager_remote.py:58-80`) polls `/agent/appium/{port}/status` for up to `stabilization_timeout_sec`. If it never returns `running=True`, the start is treated as a failed dispatch and `start_remote_temporary_node` calls `stop_remote_temporary_node` to clean up before raising.
...
## Flow B — Operator stop (`RemoteNodeManager.stop_node`)

```mermaid
sequenceDiagram
    autonumber
    participant API as API router
    participant NM as RemoteNodeManager.stop_node
    participant Agent as Host agent
    participant State as node_manager_state
    participant Alloc as appium_resource_allocator
    participant Pg as Postgres
...
## Flow C — Operator restart (`RemoteNodeManager.restart_node`)

```mermaid
sequenceDiagram
    autonumber
    participant NM as RemoteNodeManager.restart_node
    participant Agent as Host agent
    participant State as node_manager_state
    participant Pg as Postgres
...
`RESTART_BACKOFF_BASE = 2`, `RESTART_MAX_RETRIES = 3` (`node_manager.py:28-29`). After 3 failures the owner allocation is released and `NodeManagerError` propagates.
...
- `restart_node` keeps the same `owner_key` across the stop→start sequence (`node_manager.py:278`) so allocation does not flap and the agent can recognise the same owner across the restart.
- `restart_node_via_agent` (the loop-driven path) reads the existing allocation rather than creating a new one (`node_manager_remote.py:361-364`).
...
| --- | --- | --- |
| External listener on a managed port | Agent rejects start with "already in use" | Mapped to `NodePortConflictError`, manager tries next candidate port (`node_manager.py:152-158`) |
...
The `candidate_ports` helper (`node_manager_state.py:41-72`) excludes ports already held by `state=running` rows in the DB. After an unmanaged-listener conflict, the manager moves to the next free managed port. After a managed conflict that the agent could not clean up, the same retry loop applies — eventually one port wins or the manager raises `NodeManagerError("No free ports available in the configured range")`.

`restart_node_via_agent` (the loop-driven path) does **not** call `mark_node_stopped` between the agent stop and the next start; it rewrites `node.port/pid/state` in place after a successful start (`node_manager_remote.py:404-408`). Because the DB row stays `state=running` across that window, `candidate_ports` intentionally **excludes** the old `node.port` from the candidate set — the next attempt lands on a different free port. That is the desired behaviour after an unmanaged-listener conflict on the old port: rebind elsewhere, do not retry the same one (`node_manager_remote.py:381-383`).
...
`mark_node_started` (`node_manager_state.py:131-153`) and `mark_node_stopped` (`node_manager_state.py:159-187`) follow this exact order. New writers must too.

The `event_bus.publish` for `device.health_changed` is **deferred to after-commit** by `_schedule_health_event_after_commit` (`device_health_summary.py:128-174`). Subscribers must never observe a transition that did not become durable. Subscribers for `node.state_changed` see it inline, but the full transaction is committed before the subscriber's handler is awaited because publish is fire-and-forget.
...
- [ ] After any `mark_node_*`, `event_bus.publish("node.state_changed", ...)` is awaited.
