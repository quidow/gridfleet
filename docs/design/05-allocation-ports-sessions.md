...
These resources are easy to leak. Most of the bugs that look like "node won't restart" or "router still allocates a dead device" are leaks here: a port that nobody released, a WebDriver session that survived its run, or a resource claim still pinned to an orphan process. This doc captures the lifecycle for each so we know who frees what, when.
...
| **`Session` row** | DB row created at allocation time by the grid allocator (`AllocationService._claim`, `app/grid/allocation.py`, status `pending`) and promoted to `running` once the router confirms the real Appium session id | `sessions` table | recorded for the run's life + retention |
...
- The `appium_sweep` sync pass reaps `Session` rows whose Appium session no longer exists on the node.
- Run release paths that end a run abnormally (`RunLifecycleService.cancel_run`, `.force_release`, `.expire_run`, the last called through `run_reaper_loop`, all in `app/runs/service_lifecycle.py`) explicitly call `appium_direct.terminate_session(...)` against the device's Appium node for each running session before clearing the reservation. Normal `complete_run` does not terminate sessions; the test client/operator owns normal WebDriver teardown.
...
`_start_for_node` (`backend/app/appium_nodes/services/reconciler_agent.py`) calls `_start_node_on_candidate_port`, which iterates candidates until one succeeds or the pool is exhausted. The rule from commit `54707d1` (agent drops stale node state on a managed-port conflict) is what makes this loop converge: an agent that was bouncing requests on the same port should accept the next attempt instead of permanently rejecting.

The operator `restart_node` path (`ReconcilerAgentService.restart_node`) marks the old node stopped after an acknowledged stop, so the old port is available and is passed as the preferred candidate. The auto-recovery path driven by `_attempt_node_restart` (`backend/app/appium_nodes/services/node_health.py`) does **not** mark the node stopped between stop and start; the DB row keeps `desired_state=running` (and its observed process may still be live), so `candidate_ports` intentionally excludes the old port and the restart binds to a different free port. Doc 2 covers why this avoids racing an unconfirmed orphan.

## WebDriver sessions and routing

There is no per-node Grid relay and no central hub. The router (`router/`) listens on `:4444`, and for each new `POST /session` it calls the backend's internal grid API to allocate a device by capability match, then proxies that session's commands directly to the allocated device's Appium server on its managed port. The backend owns allocation and matching (`app/grid/allocation.py`); the router owns request forwarding. The backend observes live sessions directly per node: the `appium_sweep` sync pass lists each running node's Appium sessions via `appium_direct.list_sessions(node_target(device))`.
...
The backend never scrapes a hub `/status`; "what is available right now" is read straight from the `devices`/`appium_nodes` tables: `app/grid/allocation.py`'s `_eligible_devices` candidates are `operational_state = available` devices whose Appium node is viable (`node_viable_predicate`: live process, no in-flight transition) and accepting new sessions (`node_accepting_new_sessions_predicate`, the warm soft-gate).

### Reaping a WebDriver session

`appium_direct.terminate_session(target, session_id)` issues `DELETE /session/{id}` directly to the device's Appium node (`target = node_target(device)`). A 404 is treated as success (the session was already gone). Used by the run release paths in `app/runs/service_lifecycle.py`:

- `RunLifecycleService.cancel_run`
- `RunLifecycleService.force_release`
- `RunLifecycleService.expire_run` (called by `run_reaper_loop` for heartbeat/TTL expiry)

`force_release` also probes survivorship after the `DELETE` (one retry on an indeterminate result) and hard-stops the device's Appium process only for sessions that genuinely survived; a confirmed-gone session leaves the node warm instead of forcing a cold restart. `cancel_run` and `expire_run` terminate the session but never hard-stop the node.

The `appium_sweep` sync pass does not delete or create those sessions. `Session` rows are created at allocation time by the grid allocator and closed primarily by the router's `/internal/grid/sessions/ended` notification (`AllocationService.mark_ended`). The sync pass is the liveness backstop: it lists each node's live Appium sessions and, through the same `close_running_session` path, marks a row ended when its session has disappeared without a notification ever arriving.
...
sequenceDiagram
    ...
    Pg-->>Router: pending Session row (placeholder id, linked to run), device marked dirty
    Router->>Appium: proxy POST /session to allocated node
    Appium-->>Router: real Appium session id
    Router->>Pg: confirm allocation (placeholder id -> real id, status=running)
    Router-->>Client: session id
    Client->>Router: DELETE /session/{id}
    Router->>Appium: proxy DELETE /session/{id}
    Router->>Pg: notify session ended -> mark Session ended, mark device dirty
...
- `operational_state: available → busy` is the per-session change, but nobody writes it directly. The grid allocator marks the device dirty via `IntentService.mark_dirty_and_reconcile` at claim time (a `pending` Session row already counts as live), and the `device_intent_reconciler` derives `busy`. When the session ends, either the router's `/internal/grid/sessions/ended` notification or the `appium_sweep` sync pass closes the row through `close_running_session`, which marks the device dirty again; the reconciler then derives `available` or `offline`, leaving any reservation row untouched.
- The `host_sweep` node-health stage polls Appium `/status` on every running node (reserved or busy included) because the probe is a process-liveness check, not a session probe.

### Run-routed sessions

Run membership is enforced at allocation time by the backend rather than through per-worker reservation claims. Sessions bound to a run are created through the run-scoped router endpoint (`/run/{run_id}`); the router extracts the run id from the URL path and passes it to the backend's internal grid allocate call. The backend allocation gate (`_ticket_passes_reservation`, `app/grid/allocation.py`) admits a candidate only if the ticket's run id matches the device's active reservation; otherwise the candidate is rejected and the allocator moves on. Admission is strictly symmetric: a run-bound session is admitted only to devices reserved for that run, and a reserved device admits only sessions from its run.
...
| `Session` row stays `running` after the Appium session ended | `appium_sweep` sync pass skipped a tick | The pass retries on the next cycle; only escalate if persistent |
