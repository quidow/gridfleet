...
from app.appium_nodes.services.status_fold_loop import (
    DEVICE_FOLD_SECTION,
    FOLD_SECTION,
    FoldSection,
    StatusFoldLoop,
...
def _build_leader_loop_tasks(app_services: AppServices) -> list[asyncio.Task[None]]:
    ...
    status_fold = StatusFoldLoop(
        sections=(
            FoldSection(FOLD_SECTION, app_services.appium_nodes.node_health.fold_host_nodes),
            FoldSection(DEVICE_FOLD_SECTION, app_services.devices.connectivity.fold_host_devices),
        ),
