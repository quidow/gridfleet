...
class PluginService:
    ...
    async def update_plugin(self, db: AsyncSession, plugin_id: uuid.UUID, data: PluginUpdate) -> AppiumPlugin | None:
    ...
    async def fetch_host_plugins(self, host: Host) -> list[dict[str, str]]:
    ...
    async def get_host_plugin_statuses(self, host: Host, plugins: list[AppiumPlugin]) -> list[dict[str, Any]]:
        ...
        installed = await self.fetch_host_plugins(host)
