...
async def get_webhook(webhook_id: uuid.UUID, db: DbDep, webhook_services: WebhookServicesDep) -> Webhook:
    return found_or_404(await webhook_services.crud.get_webhook(db, webhook_id), "Webhook not found")
...
async def update_webhook(
    ...
    return found_or_404(await webhook_services.crud.update_webhook(db, webhook_id, data), "Webhook not found")
...
async def test_webhook(
    ...
    webhook = found_or_404(await webhook_services.crud.get_webhook(db, webhook_id), "Webhook not found")
...
async def retry_webhook_delivery(
    ...
    found_or_404(await webhook_services.crud.get_webhook(db, webhook_id), "Webhook not found")
    return found_or_404(
        await webhook_services.dispatch.retry_delivery(db, webhook_id, delivery_id), "Webhook delivery not found"
    )
