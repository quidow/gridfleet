...
from app.routers.device_route_helpers import get_device_for_update_or_404, get_device_or_404
...
@router.get("/{device_id}/test_data")
async def get_test_data(device_id: uuid.UUID, db: AsyncSession = Depends(get_db)) -> dict[str, Any]:
    device = await get_device_or_404(device_id, db)
    return await test_data_service.get_device_test_data(db, device)


@router.put("/{device_id}/test_data")
...
@router.patch("/{device_id}/test_data")
