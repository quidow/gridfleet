...
class RunReservationWriter(Protocol):
    ...
    async def restore_device_to_run(self, db: AsyncSession, device_id: uuid.UUID) -> TestRun | None: ...
...
class DeviceHealthProtocol(Protocol):
    ...
    async def apply_node_state_transition(
        ...
        device: Device,
        *,
        health_running: bool | UnsetType | None = ...,
        health_state: str | UnsetType | None = ...,
        mark_offline: bool = ...,
        revision: int | None = ...,
        observed_at: datetime | None = ...,
    ) -> None: ...
