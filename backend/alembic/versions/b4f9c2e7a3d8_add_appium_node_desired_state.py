...
def upgrade() -> None:
    nodestate_enum = postgresql.ENUM("running", "stopped", "error", name="nodestate", create_type=False)
    ...
    op.execute(
        "UPDATE appium_nodes SET desired_state = 'stopped', desired_port = NULL WHERE state IN ('stopped', 'error')"
    )
...
def downgrade() -> None:
    op.drop_constraint("ck_appium_nodes_desired_state", "appium_nodes", type_="check")
