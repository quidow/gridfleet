...
async def test_merge_device_config_coerces_bool_fields(
    db_session: AsyncSession, db_session_maker: async_sessionmaker[AsyncSession], default_host_id: str
) -> None:
    device = await create_device_record(
        db_session,
...
async def test_merge_device_config_rejects_a_missing_device(
    ...
    with pytest.raises(NoResultFound):
        async with db_session_maker() as command_db, command_db.begin():
            await service.merge_device_config(command_db, uuid.uuid4(), {"a": 1})


async def test_merge_device_config_rolls_back_mutation_audit_row_and_event(
    db_session: AsyncSession, db_session_maker: async_sessionmaker[AsyncSession], default_host_id: str
) -> None:
    device = await create_device_record(
        db_session,
        host_id=default_host_id,
        identity_value="CFG-ROLLBACK-001",
        connection_target="CFG-ROLLBACK-001",
        name="Config Rollback Device",
        pack_id=DEVICE_PAYLOAD["pack_id"],
        platform_id=DEVICE_PAYLOAD["platform_id"],
        identity_scheme=DEVICE_PAYLOAD["identity_scheme"],
        identity_scope=DEVICE_PAYLOAD["identity_scope"],
    ...
    with pytest.raises(DBAPIError):
        async with db_session_maker() as command_db, command_db.begin():
            await service.merge_device_config(command_db, device.id, {"gone": True}, "op")
            # A real aborting statement, not a patched side effect: the config
            # mutation, the audit row, and the outbox row die as one unit.
            await command_db.execute(text("SELECT 1 / 0"))

    await db_session.refresh(device)
    assert device.device_config == {}, "the config mutation outlived its rolled-back transaction"
    audit_rows = await db_session.scalar(
        select(func.count()).select_from(ConfigAuditLog).where(ConfigAuditLog.device_id == device.id)
    )
    assert audit_rows == 0, "audit row outlived its rolled-back transaction"
    events = await db_session.scalar(
        select(func.count()).select_from(SystemEvent).where(SystemEvent.type == "config.updated")
    )
