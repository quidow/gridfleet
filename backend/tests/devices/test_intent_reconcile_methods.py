...
def test_intent_service_has_reconcile_methods() -> None:
    ...
    assert list(reg.parameters) == ["self", "device_id", "intents", "publisher", "observed_reason"]
    rev = inspect.signature(IntentService.revoke_intents_and_reconcile)
    assert list(rev.parameters) == ["self", "device_id", "sources", "publisher", "observed_reason"]
