...
async def test_endpoint_health_branch_handles_top_level_failure_and_ip_ping_hysteresis(
    ...
    await _check_connectivity(db_session, settings=settings, circuit_breaker=Mock())
...
async def test_connectivity_marks_busy_device_offline(
    ...
    await device_connectivity._check_connectivity(db_session, settings=FakeSettingsReader({}), circuit_breaker=Mock())
