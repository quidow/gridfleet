...
async def test_control_plane_state_helpers_snapshot_and_reset(db_session: AsyncSession, db_host: Host) -> None:
    ...
    device = Device(
        ...
        os_version="14",
        host_id=db_host.id,
        operational_state_last_emitted=DeviceOperationalState.available,
...
async def test_operational_state_edge_publishes_only_on_change(db_session: AsyncSession, db_host: Host) -> None:
    device = Device(
        pack_id="appium-uiautomator2",
        platform_id="android_mobile",
        identity_scheme="android_serial",
