"""Schema test: deviceeventtype enum gains desired_state_changed value."""
...
async def alembic_engine() -> AsyncGenerator[AsyncEngine]:
    schema_name = f"alembic_eventtype_{uuid.uuid4().hex}"
...
@pytest.mark.db
@pytest.mark.asyncio
async def test_deviceeventtype_enum_includes_desired_state_changed(alembic_session: AsyncSession) -> None:
    res = await alembic_session.execute(
        text(
            "SELECT enumlabel FROM pg_enum e "
            "JOIN pg_type t ON t.oid = e.enumtypid "
            "JOIN pg_namespace n ON n.oid = t.typnamespace "
            "WHERE t.typname = 'deviceeventtype' AND n.nspname = current_schema()"
        )
    )
    labels = {row[0] for row in res.fetchall()}
    assert "desired_state_changed" in labels
