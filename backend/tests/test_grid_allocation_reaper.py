...
async def _seed_claimed_ticket(
    db: AsyncSession, allocation_service: AllocationService
) -> tuple[GridSessionQueueTicket, Session]:
    """A ``claimed`` ticket pointing at its freshly-minted pending Session row."""
    await seed_test_packs(db)
    await seed_host_and_running_node(db, identity=f"grid-claim-{uuid.uuid4().hex[:8]}")
    ticket = GridSessionQueueTicket(requested_body=_body(platformName="Android"))
    db.add(ticket)
    await db.flush()
    result = await allocation_service.try_allocate(db, ticket=ticket)
    assert result is not None
    await db.refresh(ticket)
    assert ticket.status == GridQueueStatus.claimed
    row = await db.get(Session, result.allocation_id)
    assert row is not None
    return ticket, row


@pytest.mark.db
async def test_reap_orphaned_claims_expires_abandoned_claim_on_ended_session(
    db_session: AsyncSession, allocation_service: AllocationService
) -> None:
    """A claimed ticket left dangling on an ended session (the expire_tickets_for_session
    seam never fired — pre-fix residue) whose client has stopped polling is queue garbage:
    the sweep terminalizes it claimed -> expired."""
...
@pytest.mark.db
async def test_reap_orphaned_claims_expires_abandoned_claim_with_null_session(
    db_session: AsyncSession, allocation_service: AllocationService
) -> None:
    """data_cleanup purges the Session -> FK SET NULL nulls session_row_id; an abandoned
    claimed ticket with no session row can never become live again."""
    ticket, _ = await _seed_claimed_ticket(db_session, allocation_service)
    ticket.session_row_id = None
    ticket.last_polled_at = datetime.now(UTC) - timedelta(seconds=60)  # abandoned
    await db_session.flush()
...
@pytest.mark.db
async def test_reap_orphaned_claims_spares_claim_on_live_session(
    db_session: AsyncSession, allocation_service: AllocationService
) -> None:
    """A claimed ticket whose Session is still pending/running is an honest in-flight claim
    even if the client paused polling — it must NOT be swept."""
    ticket, session = await _seed_claimed_ticket(db_session, allocation_service)
    assert session.status == SessionStatus.pending  # still live
...
@pytest.mark.db
async def test_reaper_cycle_sweeps_orphaned_claims_and_counts(
    db_session: AsyncSession, allocation_service: AllocationService, reaper: GridAllocationReaperLoop
) -> None:
    """Wiring: the reaper cycle runs the orphaned-claim sweep and records the metric."""
    ticket, session = await _seed_claimed_ticket(db_session, allocation_service)
    session.status = SessionStatus.error
    session.ended_at = datetime.now(UTC) - timedelta(hours=2)
    ticket.last_polled_at = datetime.now(UTC) - timedelta(seconds=60)
    await db_session.flush()
    before = GRID_ALLOCATION_OUTCOME_TOTAL.labels(outcome="orphan_claim_reaped")._value.get()

    await reaper.run_cycle(db_session)

    await db_session.refresh(ticket)
    assert ticket.status == GridQueueStatus.expired
    assert GRID_ALLOCATION_OUTCOME_TOTAL.labels(outcome="orphan_claim_reaped")._value.get() == before + 1
