...
async def test_ping_success_returns_payload_and_pooled_mode() -> None:
    with patch(
        ...
        result = await _ping_agent("1.2.3.4", 5100, pool=Mock(), circuit_breaker=Mock())
...
async def test_ping_success_reports_fresh_mode_when_pool_disabled() -> None:
    ...
    with patch("app.agent_comm.operations.agent_request", new=AsyncMock(return_value=response)):
        result = await _ping_agent("1.2.3.4", 5100, circuit_breaker=Mock())
...
def test_emit_heartbeat_log_records_full_schema() -> None:
    from app.appium_nodes.services.heartbeat import _emit_heartbeat_log
