...
from app.auth import auth_settings as process_auth_settings
...
def auth_settings(monkeypatch: pytest.MonkeyPatch) -> Iterator[dict[str, str]]:
    ...
    monkeypatch.setattr(process_auth_settings, "auth_enabled", True)
    monkeypatch.setattr(process_auth_settings, "auth_username", values["auth_username"])
    monkeypatch.setattr(process_auth_settings, "auth_password", values["auth_password"])
    monkeypatch.setattr(process_auth_settings, "auth_session_secret", values["auth_session_secret"])
    monkeypatch.setattr(process_auth_settings, "auth_session_ttl_sec", 28_800)
    monkeypatch.setattr(process_auth_settings, "auth_cookie_secure", False)
    monkeypatch.setattr(process_auth_settings, "machine_auth_username", values["machine_auth_username"])
    monkeypatch.setattr(process_auth_settings, "machine_auth_password", values["machine_auth_password"])
    yield values
...
async def test_create_from_template_returns_201(
    ...
    monkeypatch.setattr(packs_settings, "driver_pack_storage_dir", tmp_path)
