...
describe("devicesPortability", () => {
  ...
  it("posts the bundle to validate", async () => {
    ...
    expect(post).toHaveBeenCalledWith("/portability/import/validate", { schema_version: 1 });
  });
  ...
  it("posts bundle + hash + mappings to commit", async () => {
    ...
    expect(post).toHaveBeenCalledWith("/portability/import", req);
  });
});
