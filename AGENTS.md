...
The backend is a **stateless multi-worker FastAPI app**; all state is in Postgres. `app/main.py` lifespan starts 10 background loops (host_sweep, appium_sweep, durable_job_worker, run_reaper, grid_allocation_reaper, data_cleanup, fleet_capacity, pack_drain, device_intent_reconciler, background_loop_flush). They run in exactly one **scheduler process** — the prod `backend-scheduler` Compose service (one worker), or the API process itself in local/single-container runs — gated by `GRIDFLEET_RUN_BACKGROUND_LOOPS` (default `true`). A stall watchdog `os._exit(70)`s the scheduler when a loop wedges so the supervisor restarts it. Agents push one consolidated `POST /agent/hosts/status` per host on `AGENT_STATUS_PUSH_INTERVAL_SEC` (default 10 s) — nodes, restart events, start failures, pack status, host telemetry, and agent version/capabilities in a single payload; pack status has no separate channel. `host_sweep` evaluates each host's liveness from that push's recency (`general.host_offline_after_sec`, default 45 s) rather than dialing the agent, feeds the latest pushed snapshot into Appium-node convergence only for a host the recency check proved alive, then folds the push's observation sections (`node_health`, `device_health`, `device_telemetry`, `device_properties`, `host_telemetry`) into durable facts — each fold gated by a per-host stamp watermark (`host_sweep.observation_fold`) so one observation folds exactly once — and runs a cadence-gated `/agent/health` reachability probe (`general.partition_probe_interval_sec`, default 60 s) as a network-partition diagnostic only — it feeds no state. The agent gathers those observations locally on fixed cadences (30/60/300/600 s constants in `agent_app/probes.py`) against a roster pulled from `GET /agent/devices/probe-targets`. After the per-host fan-out the sweep expires stale device cooldowns. The backend dials an agent only to act, never to observe: desired-state wake hints, pack lifecycle-state polls, link-repair dispatch (+ one post-repair re-probe), presence enumeration on probe-miss, verification flows, and the partition diagnostic. The `grid_allocation_reaper_loop` expires stale router allocation tickets (devices allocated but never confirmed by the router). `appium_sweep` runs the direct-to-Appium session observation pass (liveness probes + orphan-session kill via Appium's `/appium/sessions`) on every tick or doorbell wake, then runs the scheduled session-viability pass behind a 60 s scan throttle.
...
5. The `host_sweep` folds reconcile the agent's pushed observations against DB state; `appium_sweep` keeps direct-to-Appium probes for session actions.
...
### Device row locking
Any code that writes `Device.operational_state` or `Device.lifecycle_policy_state` MUST acquire the row lock first via `app.devices.locking.lock_device` (or `lock_devices` for batch) inside the same transaction. Routers should use `get_device_for_update_or_404` for state-mutating endpoints. The `host_sweep` stages and both `appium_sweep` passes commit per device after the locked write window. The scheduler's singleton advisory lock is NOT sufficient because API mutators run on every worker and bypass it.
...
`Device.operational_state` is **derived** by `apply_derived_state` in `app.devices.services.state` (the same module as `set_operational_state`). The authoritative runtime writer is the `device_intent_reconciler` loop, which is the only loop that calls `apply_derived_state`. Observation work in the `host_sweep` stages and `appium_sweep` passes writes durable facts (health flags, session rows, `maintenance_reason`) and then calls `IntentService.reconcile_now` (or relies on the every-tick reconciler scan); it does NOT write `operational_state` directly. There are no direct (non-reconciler) callers of `set_operational_state` left: observation and lifecycle sites write durable facts (session rows, health flags, maintenance_reason, verification-lease intents) and reconcile inline only where the criterion in `docs/reference/device-lifecycle.md` ("When to reconcile inline") holds — read-your-writes, atomic intent registration, or tick-visible latency. Anything that skips the inline reconcile is re-derived by the reconciler's full scan within one `general.intent_reconcile_interval_sec` tick. Device-creation paths still set the initial value at construction time. `tests/contracts/test_no_direct_device_state_writes.py` enforces the writer contract with call-site and assignment scans. Direct attribute assignment (`device.operational_state = ...`) is forbidden outside the sanctioned writers. See `docs/reference/device-lifecycle.md`.
