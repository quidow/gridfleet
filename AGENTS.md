...
- **Conventional Commits:** all commits must use the format `type(scope): description`. Scopes: `backend`, `agent`, `frontend`, `testkit`, `docker`, `ci`, `docs`, `deps`, `deps-dev`. Use `!` for breaking changes: `feat(backend)!: description`. Enforced by commitlint in CI.
