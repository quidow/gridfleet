...
- **Ruff lint set:** backend selects `E,F,W,I,N,UP,B,A,BLE,SIM,TC,RUF,ANN,PL` — the full Pylint group, with `PLW0603` ignored (run-once `global` singleton/guard idioms), `pylint.max-args = 8`, `PLR0913` exempted per-file for routers / `protocols.py` / `agent_comm`, and the complexity/arg PL rules exempted under `tests/**`. The agent uses `E,F,W,I,N,UP,B,A,SIM,TCH,RUF,ANN,PLC0415` (no `BLE`). SQLAlchemy `Mapped[]` columns under domain model packages are exempt from `TCH003` because runtime types are required.
