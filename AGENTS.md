...
`Device.operational_state` is **derived** by `apply_derived_state` in `app.devices.services.state` (the same module as `set_operational_state`). The authoritative runtime writer is the `device_intent_reconciler` loop — it is the only loop that calls `apply_derived_state`. Observation loops (`device_connectivity`, `node_health`, `session_sync`, `session_viability`) write durable facts (health flags, session rows, `maintenance_reason`) and then call `IntentService.mark_dirty_and_reconcile` or `mark_dirty`; they do NOT write `operational_state` directly. There are four direct (non-reconciler) callers of `set_operational_state`: `app.verification.services.execution` (transient `verifying` entry state), `app.sessions.service.register_session` (sets `busy` on session registration), `app.appium_nodes.services.heartbeat`'s host-offline path (marks every device on a downed host `offline`), and `app.runs.service_lifecycle_release.release_devices` (restores the ready operational state when a run ends). The verification pass / update-failure terminals are reconciler-derived. Direct attribute assignment (`device.operational_state = ...`) is forbidden outside the sanctioned writers. See `docs/reference/device-lifecycle.md`.

These rules are enforced at runtime by a SQLAlchemy attribute-event guardrail in `backend/app/devices/services/state_write_guard.py`. The `ALLOWLIST` dict in that module is the single source of truth for which production modules may write each protected column. Any new sanctioned writer must be added to `ALLOWLIST`; unlisted callers get `StateWriteOutsideSanctionedWriterError`. Test fixtures seed state using `state_write_guard.bypass()` — production code must never call `bypass()`.

Writes to `AppiumNode.desired_state` and `AppiumNode.desired_port` MUST go through `app.appium_nodes.services.desired_state_writer.write_desired_state` under the device row lock. `AppiumNode.transition_token` and `AppiumNode.transition_deadline` are also written by `write_desired_state`, but are additionally cleared directly (per `state_write_guard.py::ALLOWLIST`) by `app.appium_nodes.routers.admin` (under the device row lock) and `app.appium_nodes.services.reconciler_agent`. Observation columns (`pid`, `port`, `active_connection_target`, `health_running`, `health_state`, `last_health_checked_at`, `last_observed_at`) are written only by sanctioned observed-state writers: the `app.appium_nodes.services.reconciler*` modules; `app.devices.services.health.apply_node_state_transition` for health fields; `app.devices.services.capability`'s active-target cache fill; verification flows in `app.verification.services.execution`; and `app.appium_nodes.services.heartbeat`'s `restart_succeeded` event handler. Operator routes and lifecycle flows write desired state only. Defer to `state_write_guard.py::ALLOWLIST` as the authoritative enumeration for these columns — with one exception: `appium_nodes.last_observed_at` is written only via a SQLAlchemy Core bulk update in `app.appium_nodes.services.reconciler` (`_touch_last_observed`), which the attribute-event guard cannot intercept (Core updates never fire ORM `set` events), so it is NOT enforceable by the guard (its ALLOWLIST entry names `reconciler`, the real writer, but is documentary only). Alembic schema/data migrations are an accepted out-of-band writer for protected columns; the runtime guard and row-lock contract apply to application code only.
...
- **Ruff lint set:** backend selects `E,F,W,I,N,UP,B,A,BLE,SIM,TCH,RUF,ANN,PLC0415`; the agent uses the same set minus `BLE` (`E,F,W,I,N,UP,B,A,SIM,TCH,RUF,ANN,PLC0415`). SQLAlchemy `Mapped[]` columns under domain model packages are exempt from `TCH003` because runtime types are required.
