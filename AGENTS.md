...
The backend is a **stateless multi-worker FastAPI app**; all state is in Postgres. `app/main.py` lifespan starts 7 background loops (host_sweep, appium_sweep, durable_job_worker, grid_allocation_reaper, device_intent_reconciler, janitor, status_fold_loop). They run in exactly one **scheduler process** — the prod `backend-scheduler` Compose service (one worker), or the API process itself in local/single-container runs — gated by `GRIDFLEET_RUN_BACKGROUND_LOOPS` (default `true`). A stall watchdog `os._exit(70)`s the scheduler when a loop wedges so the supervisor restarts it. Agents push one consolidated `POST /agent/hosts/status` per host on `AGENT_STATUS_PUSH_INTERVAL_SEC` (default 10 s) — nodes, restart events, start failures, pack status, host telemetry, and agent version/capabilities in a single payload; pack status has no separate channel. The push handler commits liveness first, then ingests restart events, converges Appium nodes, and folds the observation sections into durable facts with per-stage containment. `host_sweep` only detects silence from push recency (`general.host_offline_after_sec`, default 45 s), emits liveness edges and the offline cascade, and runs the cadence-gated `/agent/health` partition diagnostic (60 s plumbing constant). The latest `status_push.host_status` snapshot remains for host diagnostics and device-capability active-target fill. The agent gathers observations locally on fixed cadences (30/60/300/600 s constants in `agent_app/probes.py`) against a roster pulled from `GET /agent/devices/probe-targets`; fact latency is push-bounded after each probe fires. The backend dials an agent only to act, never to observe: desired-state wake hints, pack lifecycle-state polls, link-repair dispatch (+ one post-repair re-probe), presence enumeration on probe-miss, verification flows, and the partition diagnostic. The `grid_allocation_reaper_loop` expires stale queue tickets and fails crash-orphaned pending rows (session creation is backend-owned; there is no router confirm step). The `device_intent_reconciler` tick GCs expired deny intents and clears elapsed reservation-row cooldowns before its full reconcile scan. `appium_sweep` runs the direct-to-Appium session observation pass (liveness probes + orphan-session kill via Appium's `/appium/sessions`) on every tick or doorbell wake, then runs the scheduled session-viability pass behind a 60 s scan throttle.

A single **PostgreSQL advisory lock** (`app/core/leader/advisory.py`, singleton `control_plane_leader`, lock id `6001`, held for the process lifetime) guards against an accidental second loop-runner — it is a launch guard, **not** a leader election. There is no heartbeat, watcher, or preemption; failover is restart-based (compose `restart: unless-stopped` re-acquires the lock) plus the in-process stall watchdog. Dynamic-group `member_of` references live in `device_group_member_of`, whose two composite foreign keys reference `device_groups(id, group_type)` and pin each endpoint's kind (`dynamic` source, `static` target). The target FK is `ON DELETE RESTRICT`, so a static group with live referrers cannot be deleted; the source FK is `ON DELETE CASCADE`, so deleting a dynamic group drops its edges. There is no application-side reference scan and no group-definition advisory lock.

Definition writers obey one lock order: **parent `device_groups` rows before `device_group_member_of` edges.** `delete_group` takes `FOR UPDATE` on its target as its first statement; `update_group` locks its source row through `_load_group_for_mutation`; `_replace_member_of` takes `FOR KEY SHARE` on each target before touching edges. `create_group` obeys the order only transitively, through `_replace_member_of` — it never locks a row itself. The portability importer does not obey it at all: `_insert_member_of_references` inserts the edge tuple first and only then does the FK trigger take its `FOR KEY SHARE` on the parent. It is safe only because `_validate_group_references` rejects any bundle whose keys already exist in the target, so both endpoints of every edge are minted inside the same uncommitted transaction and no peer can be the foreign parent; if the importer is ever changed to reference pre-existing static groups, that exemption stops holding and it would need the same ordering as the other writers. That order is what keeps `update_group` and `delete_group` from deadlocking through edge tuple locks — an earlier attempt that hoisted the RESTRICT trigger's own edge lock instead only narrowed the window, because the trigger takes edges *after* the parent and covers edges that appear later. Because the target row is locked before the dependent read, that read is authoritative and the RESTRICT violation is unreachable from application code; the foreign key remains the backstop for raw SQL and migrations. If a future change ever drops that `FOR UPDATE`, the failure mode is a 500 rather than a 409 — `test_definition_writers_lock_parents_before_touching_edges` and the delete race tests are what hold the line.

Membership writes (`add_members`, `remove_members`) keep their own `device_groups` row lock via `_get_group_row(..., for_update=True)`. It is not the duplicate-insert guard — `INSERT ... ON CONFLICT DO NOTHING` makes a concurrent duplicate benign on its own, and `test_bug_audit_group_add_members_race.py` (which pins that) passes with the lock removed. What the lock buys is an authoritative read against a concurrent `delete_group`, which locks the same row: without it `add_members` inserts against a doomed parent and the foreign-key check turns a 404 into a 500, while `remove_members` reports `removed 0` for a group that no longer exists. `test_concurrency_group_membership_writers_vs_delete.py` pins both and fails if either lock is dropped.

Declare new advisory-lock ids in `app/core/leader/advisory.py` — `6001` (`control_plane_leader`) is the only one declared as a named constant, but the single-bigint keyspace it lives in still has two occupants, not one: the host-scoped Appium port lock in `app/appium_nodes/services/resource_service.py` also takes it, via `pg_advisory_xact_lock(hashtextextended(...))` rather than a literal id. When adding a new periodic task, add a `BackgroundLoop` subclass to the roster in `_build_leader_loop_tasks` (`app/main.py`) and expose Prometheus gauges via `observe_background_loop`, rather than spawning bare `asyncio.create_task` loops. Domain-specific loops live under their owning `app/<domain>/services/` package. Scheduling doctrine: a **BackgroundLoop** per independent lifecycle; **stage_due stages** only as sub-cadences of an owning sweep (host_sweep's partition probe; the janitor's run_reaper/fleet_capacity/pack_drain-backstop/pack_artifact_reaper/release_rollout/data_cleanup/heartbeat-flush stages, base tick 15 s). Stage cadences are plumbing constants, never registry settings. Pack drain is event-driven: session/run release paths call `complete_drain_if_draining` inline; the janitor stage is the backstop.
...
A runtime guard (`backend/tests/contracts/device_lock_guard.py`) hooks SQLAlchemy ORM events across every `db`-marked backend test and raises `DeviceLockGuardViolation` for any decision-fact write (`DeviceIntent`, `Session`, `DeviceReservation`, `DeviceRemediationLogEntry`, plus the two `Device` ledger columns) whose transaction does not hold that device's row lock. Four proof modes in `test_no_direct_device_state_writes.py` cover every registered writer: `accepts_locked` and `acquires_locked` prove a device row lock was taken; `guarded_update` proves predicate authority instead — an ID-based compare-and-swap whose own WHERE cannot apply twice; `fleet_retention` proves a fleet-wide retention DELETE decides nothing, by re-deriving an already-elapsed age cutoff plus a dead-row predicate on a different column. Read `device_lock_guard.py`'s module docstring for the exemptions, the flush-misattribution remedies, and the disclosed ceilings. If `DeviceLockGuardViolation` fires, take the lock via `app.devices.locking` before the write, or check whether the write instead qualifies for one of the other three modes.
...
- **Inject a real failure, not a mock.** A method patched with `side_effect` leaves the session clean; a real failing statement leaves the transaction aborted, which is a different code path. Tests for error handling must trigger the error the way production does — a real statement, a real timeout, a real constraint violation. A `db.rollback()` that an error path genuinely required was missed for exactly this reason.
- **Re-derive expected values independently.** When a test asserts a computed number (a backoff sequence, a statement count, a retry total), derive it a second time by a different method — a standalone simulation run against the committed source, not a hand trace of it. Two arithmetic errors in one backoff loop survived a hand trace and a confident narrative, and died to a simulation.
