...
The backend is a **stateless multi-worker FastAPI app**; all state is in Postgres. `app/main.py` lifespan starts 10 background loops (host_sweep, appium_sweep, durable_job_worker, run_reaper, grid_allocation_reaper, data_cleanup, fleet_capacity, pack_drain, device_intent_reconciler, background_loop_flush). They run in exactly one **scheduler process** — the prod `backend-scheduler` Compose service (one worker), or the API process itself in local/single-container runs — gated by `GRIDFLEET_RUN_BACKGROUND_LOOPS` (default `true`). A stall watchdog `os._exit(70)`s the scheduler when a loop wedges so the supervisor restarts it. `host_sweep` fetches `/agent/health` once per host per tick, applies the heartbeat/liveness verdict, feeds that exact payload into Appium-node convergence only for a freshly alive host, then runs node health as a cadence-gated per-host stage (`general.node_check_interval_sec`, see `stage_due`) for that host; after the per-host fan-out it runs the cadence-gated **global** stages in order (connectivity `general.device_check_interval_sec`, host_resource_telemetry, hardware_telemetry, property_refresh — each gated by its own interval setting, see `SweepStage`) against the host statuses the same cycle just wrote. The `grid_allocation_reaper_loop` expires stale router allocation tickets (devices allocated but never confirmed by the router). `appium_sweep` runs the direct-to-Appium session observation pass (liveness probes + orphan-session kill via Appium's `/appium/sessions`) on every tick or doorbell wake, then runs the scheduled session-viability pass behind a 60 s scan throttle.
...
5. The `host_sweep` node-health and connectivity stages reconcile what the agent reports against DB state, using direct-to-Appium probes.
