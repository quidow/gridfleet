...
Writes to `Device.operational_state` and `Device.hold` are **derived** by `apply_derived_state` in `app.devices.services.state_derivation`, which calls `set_operational_state` / `set_hold` from `app.devices.services.state`. The authoritative runtime writer is the `device_intent_reconciler` loop — it is the only loop that calls `apply_derived_state`. Observation loops (`device_connectivity`, `node_health`, `session_sync`, `session_viability`) write durable facts (health flags, session rows, `maintenance_reason`) and then call `IntentService.mark_dirty_and_reconcile` or `mark_dirty`; they do NOT write `operational_state` or `hold` directly. `DeviceStateMachine.transition` is no longer called by observation loops. The one remaining direct caller is the `verification_execution` job, for the transient `verifying` entry state and the node-cleanup-failure offline terminal (both route through `set_operational_state`); the normal verification pass / update-failure terminals are reconciler-derived. Direct attribute assignment (`device.operational_state = ...`) is forbidden outside the sanctioned writers. See `docs/reference/device-lifecycle.md`.
