...
Membership writes (`add_members`, `remove_members`) keep their own `device_groups` row lock via `_get_group_row(..., for_update=True)`, which serialises concurrent membership edits against each other (the duplicate-insert race in `test_bug_audit_group_add_members_race.py`).
