...
Writes to `AppiumNode.desired_state`, `AppiumNode.desired_port`, `AppiumNode.transition_token`, and `AppiumNode.transition_deadline` MUST go through `app.services.desired_state_writer.write_desired_state` under the device row lock. Observed Appium node columns (`state`, `port`, `pid`, `active_connection_target`) are written only by the sanctioned observed-state writers that already acquire the device/node locks: `app.services.node_service.mark_node_started`, `mark_node_stopped`, and `restart_node_via_agent`; `app.services.appium_reconciler` convergence writers; `app.services.device_health.apply_node_state_transition`; `app.services.capability_service`'s active-target cache fill; `app.services.device_verification_execution.retain_verified_node`; lifecycle crash handling in `app.services.lifecycle_policy_actions`; and `app.services.heartbeat`'s `restart_succeeded` event handler (updates `pid` after the agent confirms a restart). New operator routes and lifecycle flows should write desired state only and let the reconciler/observed-state writers update runtime columns.
